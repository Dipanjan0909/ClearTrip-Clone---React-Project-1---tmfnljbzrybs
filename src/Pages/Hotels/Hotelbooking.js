...
const Hotelbooking = () => {
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div>
        <div
          className="hotel-booking-container"
          style={hoelData ? { display: "flex" } : {}}
        >
          <div
            className="hotel-form"
            style={hoelData ? { display: "flex", flexDirection: "row" } : {}}
          >
            <label>
              Location:
              <input
            ...
            <button onClick={handleSearch} style={hoelData?{borderRadius:"12px",height:"60px",width:"120px"}:{}}>Search Hotels</button>
          </div>
          <div>
            {hoelData &&
              hoelData.map((item, i) => <HotelDetails {...item} key={i} />)}
          </div>
        </div>
      </div>
    </div>
